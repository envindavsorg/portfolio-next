...
import { NextAppStructure, NextAppStructureOG } from '../constants/NextAppStructure';
...
<Alert className="mb-6">
    <AlertDescription className="flex items-center gap-3">
        <InfoIcon className="size-6 shrink-0 text-theme" />{' '}
...
Après la création de votre application, et l'installation de toutes les dépendances,
vous devriez avoir cette structure de dossier :

export // Recursive component to render the folder/file structure
const RenderTree = ({ elements }) => (
    <>
        {elements.map((element) => (
            element.children ? (
                <Folder key={element.id} value={element.id} element={element.name}>
                    <RenderTree elements={element.children} /> {element.comment && <span className="text-xs text-neutral-600 dark:text-neutral-400">{element.comment}</span>}
                </Folder>
            ) : (
                <File key={element.id} value={element.id}>
                    {element.name} {element.comment && <span className="text-xs text-neutral-600 dark:text-neutral-400">{element.comment}</span>}
                </File>
            )
        ))}
    </>
...
<div className="my-6 relative flex h-80 sm:h-96 w-full flex-col items-center justify-center overflow-hidden rounded-md border bg-background border-neutral-200 dark:border-neutral-700">
    <Tree initialExpandedItems={[ "1", "2" ]} elements={NextAppStructure}>
        <>
            {NextAppStructure.map((element) => (
                element.children ? (
                    <Folder key={element.id} value={element.id} element={element.name}>
                        <RenderTree elements={element.children} />
                    </Folder>
                ) : (
                    <File key={element.id} value={element.id}>
                        {element.name}
...
<div className="my-6 relative flex h-80 sm:h-96 w-full flex-col items-center justify-center overflow-hidden rounded-md border bg-background border-neutral-200 dark:border-neutral-700">
    <Tree initialExpandedItems={[ "1", "19" ]} elements={NextAppStructure}>
        <>
            {NextAppStructureOG.map((element) => (
                element.children ? (
                    <Folder key={element.id} value={element.id} element={element.name}>
                        <RenderTree elements={element.children} />
                    </Folder>
