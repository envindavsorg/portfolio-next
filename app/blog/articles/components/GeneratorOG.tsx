...
export const GeneratorOG = ({ name }: GeneratorOGProps): React.JSX.Element => {
	const [imageUrl, setImageUrl] = useState(
		`https://www.cuzeacflorin.fr/api/og?heading=${encodeURIComponent(name)}&type=image&mode=dark`,
	);
	...
	const onSubmit = async (data: z.infer<typeof FormSchema>): Promise<void> => {
		...
		try {
			...
			const url: string = `https://www.cuzeacflorin.fr/api/og?heading=${encodedTitle}&type=image&mode=dark`;
			setImageUrl(url);
	...
	return (
		<div className="my-6 flex flex-col gap-y-3 rounded-md border border-neutral-200 p-4 dark:border-neutral-700">
			<div className="flex items-center gap-x-6 sm:mb-3 sm:gap-x-6">
				<TextAaIcon className="size-8 shrink-0" weight="bold" />
				<ArrowRightIcon className="size-4 shrink-0" weight="regular" />
			...
			<Form {...form}>
				...
				>
					...
					/>
					<button
						className="ms-auto h-11 w-fit rounded-md border border-neutral-200 bg-background px-3 py-2 dark:border-neutral-700"
						type="submit"
					>
						{isLoading ? (
