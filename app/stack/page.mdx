...
export const Stars = async () => {
	...
	return (
		<div className="flex w-full flex-col space-x-0 space-y-3 sm:flex-row sm:space-x-3 sm:space-y-0">
			{data.map(({ avatar, name, link, stars }, idx) => (
				<Channel
					key={`${name}-${idx}`}
					avatar={avatar}
					name={`@${name}`}
...
<Motion className="mt-6 pb-[52px]" variants={variantsThree}>
...
<Motion className="!text-lg sm:!text-xl mt-6" variants={variantsSix} asChild>
