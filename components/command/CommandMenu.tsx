...
import { contactItems, linksItems } from '@/components/contact/Data';
import type { NavItems } from '@/components/navigation/Data';
					...
					<CommandGroup className="mt-4">
						<Card>
							...
							<CardContent className="px-3 pt-0 pb-3">
								{navItems.map(({ desc, link, icon }, idx: number) => (
									<CommandItem
										key={`link-${idx}`}
										onSelect={() => {
											setOpen(false);
									...
									>
										<div className="*:size-4 *:text-theme">{icon}</div>
										<span>{desc}</span>
									</CommandItem>
								))}
							</CardContent>
