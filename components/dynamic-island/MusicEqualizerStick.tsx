...
const MusicEqualizerStick = ({ baseLength, colors }: Props) => {
	...
	const red = Math.max(Number.parseInt(primaryColor.slice(0, 2), 16) ?? 0, 60);
	const green = Math.max(
		Number.parseInt(primaryColor.slice(2, 4), 16) ?? 0,
		60,
	);
	const blue = Math.max(Number.parseInt(primaryColor.slice(4, 6), 16) ?? 0, 60);

	return (
		<motion.div
			className={`col-span-1 mx-auto my-auto h-6 rounded-full`}
			style={{
