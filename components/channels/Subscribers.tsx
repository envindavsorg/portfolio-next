...
export const SubscribersChannel = async (): Promise<React.JSX.Element> => {
	...
	return (
		...
		>
			<div className="mt-6 flex w-full flex-col space-x-0 space-y-3 sm:flex-row sm:space-x-3 sm:space-y-0">
				{subscribers.map(({ avatar, name, link, metric, icon }, idx: number) => (
							...
							<div className="flex items-center space-x-4">
								<div className="relative">
									...
									<div className="-bottom-0.5 -right-1.5 absolute rounded-full bg-foreground p-1 *:size-3 *:text-background">
										{icon === 'github' && <GithubLogoIcon weight="duotone" />}
										{icon === 'linkedin' && <LinkedinLogoIcon weight="duotone" />}
									</div>
								</div>
